...
namespace OJT_MT
{
    public partial class AccountSettingsForm : Form
    {
        ...
        {
            string currentPassword = OurCustomUtils.PassHash(textBoxCurrentPassword.Text);
            var dbHelper = new DatabaseHelper();
            if (await dbHelper.IsPasswordCorrectAsync(_userID, currentPassword)) //Check if current password is correct
            {
                if (currentPassword == OurCustomUtils.PassHash(textBoxNewPassword.Text))//Check if current password is the same as new password
                {
                ...
                {
                    string newPassword = OurCustomUtils.PassHash(textBoxNewPassword.Text);
        ...
        private async void SaveImage_ClickAsync(object sender, EventArgs e)
        {
            ...
            byte[] newPfp = OurCustomUtils.ImageToLongBlob(pictureBoxUserImage.Image);
