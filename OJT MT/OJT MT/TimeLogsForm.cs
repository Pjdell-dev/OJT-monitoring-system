    ...
    public partial class TimeLogsForm : Form
    {
        ...
        public async Task LoadStudentTimeLogs()
        {
            try
            {
                ...
                string query = "SELECT time_in.date AS 'Date', time_in.time_in AS 'Time In', time_out.time_out AS 'Time Out', TIMEDIFF(time_out.time_out, time_in.time_in) AS 'Total Hours' FROM time_in JOIN time_out ON time_in.date=time_out.date AND time_in.student_id=time_out.student_id WHERE time_in.student_id = @student_id;";
