...
namespace OJT_MT
{
    public partial class StudentAccomplishmentReports : Form
    {
        ...
        private async Task LoadStudentAccomplishmentReports()
         {
             try
             {
                 ...
                 viewReportButtonColumn.Width = 150;
        ...
        private async void btnViewFile_Click()
        {
            try
            {
                ...
                string idQuery = "SELECT content FROM accomplishment_reports WHERE student_id = @student_id";

                using var dbHelper = new DatabaseHelper();
                var parameters = new MySqlParameter[]
                {
                    new MySqlParameter("@student_id", _studentId)
                };
